...
namespace TreeTest
{
	...
	public:
		...
		template<typename T> using ContainerTemplate = AVLTree<T, Mypred<T>>;

		TEST_METHOD(TreeSizeTest)
		{
			//  Тестирование размера дерева при разных сценариях использования
			ContainerTemplate<int> T1;
			Assert::AreEqual(T1.size(), size_t(0), L"Пустое дерево имеет ненулевой размер!");
			ContainerTemplate<int> Tree = { 40,50,30,35,10,75,23,87,68 };
			Assert::AreEqual(Tree.size(), size_t(9), L"Неверно указывается размер дерева!");
